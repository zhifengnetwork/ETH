...
class We7_couponModuleSite extends WeModuleSite {
		...
		if ($op == 'post') {
			...
			if (!empty($id)){
				...
				if (!empty($clerk['uid'])) {
					...
					$menu_permission = pdo_getcolumn('users_permission', array('uniacid' => $_W['uniacid'], 'uid' => $clerk['uid'], 'type' => 'system'), 'permission');
					if (!empty($menu_permission)) {
						$user_menu_permission = explode('|', $menu_permission);
					}
					$clerk['permission'] = $user_menu_permission;
				}
			} else {
				$clerk = array(
					'permission' => array()
			...
			if (!empty($permission) && is_array($permission)) {
				foreach ($permission as $key => $value) {
					if ($value['group_name'] == 'stat' || $value['group_name'] == 'paycenter') {
	...
	public function doWebClerkdesk() {
		...
		if ($op == 'index') {
			...
			if (!empty($permission) || is_array($permission)) {
				foreach ($permission as $key => $value) {
					if ($value['title'] == '数据统计' || $value['title'] == '收银台') {
						unset($permission[$key]);
					}
				}
			}
