...
class Member_EweiShopV2Model
{
	...
	public function level12($openid, $money)
	{
		...
		if ($credit1 > $sys['bibi'])  return -1;

		//查询 当前会员的动态等级和管理等级
		$level1 = pdo_fetch("select *  from " . tablename("ewei_shop_commission_level") . "where uniacid=:uniacid and id = :id ", array(':uniacid' => $_W['uniacid'], ':id' => $member['agentlevel1']));

		$level2 = pdo_fetch("select *  from " . tablename("ewei_shop_commission_level2") . "where uniacid=:uniacid and id = :id ", array(':uniacid' => $_W['uniacid'], ':id' => $member['agentlevel2']));

		//查询投资后的余额达到的动态等级和管理等级
		$levels1 = pdo_fetch("select *  from " . tablename("ewei_shop_commission_level") . "where uniacid=:uniacid and start<=:credit1 and end>=:credit1 ", array(':uniacid' => $_W['uniacid'], ':credit1' => $credit1));

		$levels2 = pdo_fetch("select *  from " . tablename("ewei_shop_commission_level2") . "where uniacid=:uniacid and start<=:credit1 and end>=:credit1 ", array(':uniacid' => $_W['uniacid'], ':credit1' => $credit1));

		if ($level1 && $levels1) {
			if ($level1['type'] < $levels1['type'])
				pdo_update('ewei_shop_member', array('agentlevel' => $levels1['id']), array('uniacid' => $_W['uniacid'], 'openid' => $openid));
		} else if ($levels1) {
			pdo_update('ewei_shop_member', array('agentlevel' => $levels1['id']), array('uniacid' => $_W['uniacid'], 'openid' => $openid));
		}

		if ($level2 && $levels2) {
			if ($level2['type'] < $levels2['type'])
				pdo_update('ewei_shop_member', array('agentlevel2' => $levels2['id']), array('uniacid' => $_W['uniacid'], 'openid' => $openid));
		} else if ($levels2) {
			pdo_update('ewei_shop_member', array('agentlevel2' => $levels2['id']), array('uniacid' => $_W['uniacid'], 'openid' => $openid));
		}
	...
	public function checkMember()
	{
		...
		if ($followed || empty($shopset['shop']['getinfo']) || ($shopset['shop']['getinfo'] == 1)) {
			...
			$mc['avatar'] = rtrim($userinfo['headimgurl'], '0123') . "132";
