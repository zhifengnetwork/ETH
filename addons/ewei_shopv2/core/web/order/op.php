...
class Op_EweiShopV2Page extends WebPage 
{
	...
	public function finish() 
	{
        ...
        $ass = pdo_fetchall("select o.openid,og.*  from ".tablename("ewei_shop_order")."o left join ".tablename("ewei_shop_order_goods")."og on o.id=og.orderid "." where o.uniacid=:uniacid  and o.id=:id  ",array(':uniacid' => $_W['uniacid'],':id'=>$item['id']));
        foreach ($ass as $key => $value) {
            m('common')->comm($value['id'],$item);   //传订单号和购买人信息
        }
