...
class Index_EweiShopV2Page extends WebPage 
{
	public function main() 
	{
        ...
        else if (cv('sysset.payset1'))
        {
            header('location: ' . webUrl('sysset/payset1'));
        }
	...
	public function shop() 
	{
		...
		$ass = pdo_fetch("select kefu,wx,wxkf from ".tablename("ewei_shop_sysset")." where uniacid=:uniacid ",array(':uniacid'=>$_W['uniacid']));
		if ($_W['ispost'])
		{
			...
			//商城支付方式
			$zf['add'] = $_POST['data']['add'];
			$zf['give'] = $_POST['data']['give'];
			$zf['zhuanzhangsxf'] = $_POST['data']['zhuanzhangsxf'];
			$zf['guamaiis'] = $_POST['data']['guamaiis'];
			$zf['bibi'] = $_POST['data']['bibi'];
            $zf['wx'] = $_POST['data']['wx'];
            $zf['weixinfile'] = save_media($data['weixinfile']);
            $zf['kefu'] = $_POST['data']['kefu'];
            $zf['kefufile'] = save_media($data['kefufile']);
            $zf['wxkf'] = $_POST['data']['wxkf'];
            $zf['wxkffile'] = save_media($data['wxkffile']);
            pdo_update("ewei_shop_sysset",$zf,array('uniacid'=>$_W['uniacid']));
