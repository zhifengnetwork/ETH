...
class Op_EweiShopV2Page extends MobileLoginPage 
{
	...
	public function finish() 
	{
        ...
        $member = pdo_fetch("select * from ".tablename("ewei_shop_member")."where uniacid=:uniacid and openid=:openid",array(':uniacid'=>$_W['uniacid'],':openid'=>$order['openid']));
        $order['agentid'] = $member['agentid'];
       
		pdo_update('ewei_shop_order', array('status' => 3, 'finishtime' => time(), 'refundstate' => 0), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
        // -----------分销---------   防止用户添加到购物车在付款
        $ass = pdo_fetchall("select o.openid,og.*  from ".tablename("ewei_shop_order")."o left join ".tablename("ewei_shop_order_goods")."og on o.id=og.orderid "." where o.uniacid=:uniacid  and o.id=:id  ",array(':uniacid' => $_W['uniacid'],':id'=>$order['id']));
        foreach ($ass as $key => $value) {
            m('common')->comm($value['id'],$order);   //传订单号和购买人信息
        }
        // -----------分销---------
