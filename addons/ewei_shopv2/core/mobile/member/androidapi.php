...
class Androidapi_EweiShopV2Page extends MobilePage
{
	...
	public function guamai_appeal_list()
	{
		...
		// $user_id = $_W['openid'];
		// $users = pdo_fetch("select * from" . tablename("ewei_shop_member") . " where openid='$user_id'");
		$id = $_GPC['id'];
		$guamai_appeal = pdo_fetch("select g.*,m.* from" . tablename("guamai_appeal") . ' g left join ' . tablename('guamai') . '  m ON m.id=g.order_id' . " where g.id='$id'");
	...
	public function investment_record()
	{
		...
		$psize = 10;
		$openid = $_W['openid'];
		$list =  pdo_fetchall("select g.*,m.nickname from" . tablename("ewei_shop_member_log") . "g left join" . tablename("ewei_shop_member") . "m on g.openid=m.openid" . " where g.uniacid=:uniacid and g.type in('$type','$type2') and g.openid=:openid order by g.createtime desc" . ' LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, array(':uniacid' => $_W['uniacid'], ':openid' => $_W['openid']));
	...
	public function article_detail()
	{
		...
		if (empty($article)) {
			returnJson(array(), '文章不存在!', -2);
		}

		$article['article_content'] = p('article')->mid_replace(htmlspecialchars_decode($article['article_content']));
		...
		if (!empty($openid) && (is_weixin() || is_h5app()) && !empty($myid)) {
			...
			if (empty($article['article_areas'])) {
				...
				p('article')->doShare($article['id'], $shareid, $myid);
			}
		} else {
		...
		if (!empty($advs)) {
			foreach ($advs as $i => $v) {
				$advs[$i]['link'] = p('article')->href_replace($v['link']);
			}
		}

		$article['product_advs_link'] = p('article')->href_replace($article['product_advs_link']);
		$article['article_linkurl'] = p('article')->href_replace($article['article_linkurl']);

		$art['product_advs_link'] = $article['product_advs_link'];
		$art['article_linkurl'] = $article['article_linkurl'];
		$art['product_advs_more'] = $article['product_advs_more'];
		$art['product_advs_title'] = $article['product_advs_title'];
		$art['product_advs_type'] = $article['product_advs_type'];
		$art['article_report'] = $article['article_report'];
		$art['article_content'] = $article['article_content'];
		$art['article_areas'] = $article['article_areas'];
		$art['article_mp'] = $article['article_mp'];
		$art['article_date_v'] = $article['article_date_v'];
		$art['article_author'] = $article['article_author'];
		$art['article_title'] = $article['article_title'];
	...
	public function reg_updpwd()
	{
		...
		if ($type == 'sms_changepwd') {
			...
			$data = array('uniacid' => $_W['uniacid'], 'mobile' => $mobile, 'nickname' => $nickname, 'openid' => $openid, 'pwd' => md5($pwd . $salt), 'salt' => $salt, 'credit4' => $credit4, 'createtime' => time(), 'mobileverify' => 1, 'comefrom' => 'mobile', 'agentid' => $agentid);
