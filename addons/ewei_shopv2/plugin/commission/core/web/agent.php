...

{
...

	{
		...
		$sql = 'select dm.*,dm.nickname,dm.avatar,l.levelname as levelname,l2.levelname as levelname2,l3.levelname as levelname3,p.nickname as parentname,p.avatar as parentavatar,f.follow as followed, f.unfollowtime from ' . tablename('ewei_shop_member') . ' dm ' . ' left join ' . tablename('ewei_shop_member') . ' p on p.id = dm.agentid ' . ' left join ' . tablename('ewei_shop_commission_level') . ' l on l.id = dm.agentlevel' . ' left join ' . tablename('ewei_shop_commission_level2') . ' l2 on l2.id = dm.agentlevel2'. ' left join ' . tablename('ewei_shop_commission_level3') . ' l3 on l3.id = dm.agentlevel3'  . ' left join ' . tablename('mc_mapping_fans') . 'f on f.openid=dm.openid and f.uniacid=' . $_W['uniacid'] . ' where dm.uniacid = ' . $_W['uniacid'] . ' and dm.isagent =1  ' . $condition . ' ORDER BY dm.agenttime desc';
