...
class Level2_EweiShopV2Page extends PluginWebPage

{
...

	{
		...
		$others = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_commission_level2') . ' WHERE uniacid = \'' . $_W['uniacid'] . '\' ORDER BY commission1 asc');
...

	{
		...
		if ($id == 'default') {
			...
			$level = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_commission_level2') . ' WHERE id=:id and uniacid=:uniacid limit 1', array(':id' => intval($id), ':uniacid' => $_W['uniacid']));
		...
		if ($_W['ispost']) {
			...
			$data = array('uniacid' => $_W['uniacid'], 'levelname' => trim($_GPC['levelname']),'type' => trim($_GPC['type']),'start' => trim($_GPC['start']),'end' => trim($_GPC['end']), 'commission1' => trim(trim($_GPC['commission1']), '%'), 'commission2' => trim(trim($_GPC['commission2']), '%'), 'commission3' => trim(trim($_GPC['commission3']), '%'), 'commissionmoney' => trim($_GPC['commissionmoney'], '%'), 'ordermoney' => $_GPC['ordermoney'], 'ordercount' => intval($_GPC['ordercount']), 'downcount' => intval($_GPC['downcount'])
			...
			if (!empty($id)) {
				...
				if ($id == 'default') {
					...
					pdo_update('ewei_shop_commission_level2', $data, array('id' => $id, 'uniacid' => $_W['uniacid']));
				...
				pdo_insert('ewei_shop_commission_level2', $data);

				$id = pdo_insertid();

				plog('commission.level2.add', '添加分销商等级 ID: ' . $id);

			}



			show_json(1, array('url' => webUrl('commission/level2')));
