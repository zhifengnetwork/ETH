...
if (!class_exists('CommissionModel')) {
...

	{
		...
		public function checkAgent1($openid = ''){
			...
			$arr = pdo_fetch("select * from".tablename("ewei_shop_member")."where uniacid=:uniacid and id=:id ",array(':uniacid'=>$_W['uniacid'],':id'=>$agentid));

			pdo_update('ewei_shop_member', array('clickcount' => $arr['clickcount'] + 1), array('uniacid' => $_W['uniacid'], 'id' => $agentid));

			// $ass = pdo_fetch("select count(id) as sum from".tablename("ewei_shop_member")."where uniacid=:uniacid and agentid=:agentid ",array(':uniacid'=>$_W['uniacid'],':agentid'=>$agentid));

			$arr2 = $this->lingdaolevel();
	...
	public function lingdaolevel(){
		...
		foreach ($member as $key => $val) {
			...
			// dump("select *  from ".tablename("ewei_shop_commission_level3")."where uniacid=:uniacid and ordercount<=:clickcount and downcount<=:tuandui order by type desc ",array(':uniacid'=>$_W['uniacid'],':clickcount'=>$nums_tuijian,':tuandui'=>$tuandui));
			// dump($val['id']);
			// dump($levels1['id']);
