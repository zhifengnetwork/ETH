...
<div class='fui-page  fui-page-current member-log-page'>
  ...
  <script id="tpl_maichu" type="text/html">
      ...
      <% each list as val %>
        <li class="lis">
          ...
          <% /if %>
          <% if val.status == 2 %>
          <div class="maiRu_btn" data-id='<% val.id %>' onclick="alert('该账单已交易完成')">交易完成</div>
          <% /if %>
        </li>
      <% /each %>
      </ul>
  ...
  <script id="tpl_mairu" type="text/html">
    ...
    <% each list as val %>
        <li class="lis">
          ...
          <% if val.status == 0 %>
          <div class="maiChu_btn" data-id="<% val.id %>" style="background-color: #429547"
  ...
  $('.container').on('click','.maiChu_btn',function (e) {
    ...
    // $('.container').on('click','.maiRu_btn',function (e) {
    //   e.stopPropagation();
    //   let openid = $('.maiRu_btn').data('openid');
    //   location.href="<?php  echo mobileurl('member/guamai/sellout'),array('openid'=>"+ openid +")?>"
    // })
    ...
    $('.maiChu_price').bind('input onpropertychange',function () {
      ...
      if($('.maiChu_Num').val() != '' && $('.maiChu_price').val() != ''){
        ...
        setTrx = Number(getMoney) + Number($('.sxf0').html());
        $('.setTrx0').val(setTrx);
      } else {
        $('.getMoney0').val('0');
        $('.setTrx0').val("0");
    ...
    $('.maiRu_price').bind('input onpropertychange',function () {
      ...
      if($('.buyNum').val() != '' && $('.maiRu_price').val() != ''){
        let getMoney = $('.buyNum').val() * $('.maiRu_price').val();
    ...
    // 点击加号弹出弹框
    $('.fui-header-right').click(function () {
      // console.log(typeTab);
      if (typeTab == 1) {
        $('.mask0').animate({
          bottom: "0"
  ...
  <script language='javascript'>
    window._type = 1;
    require(['biz/member/guamaijilu'], function (modal) {
