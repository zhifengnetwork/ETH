		...
		<div class="we7-select-msg">
			<ul class="tab-navs">
				...
				<li class="tab-nav tab-img <?php  if($options['image']) { ?>hidden<?php  } ?>">
					<a href="javascript:void(0);" onclick="select_mediaid('image');">&nbsp;<i class="wi wi-img"></i><span class="msg-tab-title">微信图片</span></a>
				</li>
...
<script>
	...
	window.select_mediaid = function(type, otherVal){
		var option = {
			...
			others: {
				basic: {
					typeVal: angular.isDefined(otherVal) ? otherVal : null,
				},
				news: {
					showwx: angular.isDefined(otherVal) ? otherVal === 'wx' ? true : false : true,
					showlocal: angular.isDefined(otherVal) ? otherVal === 'local' ? true : false : true
				}
			}
		};
		if (type == 'module') {
