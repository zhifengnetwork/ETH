	...
	<div class="ext-apply-list clearfix">
		...
		<div class="ext-apply-item <?php  if($row['shortcut']) { ?>apply-show<?php  } ?>" >
			...
			<div class="mask">
				...
				<?php  if($row['shortcut']) { ?>
				<a href="<?php  echo url('profile/module/shortcut', array('modulename' => $row['name'], 'shortcut' => STATUS_OFF))?>" onclick="return ajaxopen(this.href);" class="cut-btn" title="移出菜单">
					<i class="wi wi-remove"></i>
				</a>
				<?php  } else { ?>
				<a href="<?php  echo url('profile/module/shortcut', array('modulename' => $row['name'], 'shortcut' => STATUS_ON))?>" onclick="return ajaxopen(this.href);" class="cut-btn" title="显示到菜单">
					<i class="wi wi-eye"></i>
				</a>
				<?php  } ?>
				<a href="<?php  echo url('profile/module/top', array('modulename' => $row['name']))?>" class="stick">
