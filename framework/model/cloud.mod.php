...
function cloud_m_prepare($name) {
	...
	$pars['module'] = $name;
	//$dat = cloud_request('hhttp://24.hskj2018.com/pull.php', $pars);
	if (is_error($dat)) {
		return $dat;
	}
...
function cloud_sms_send($mobile, $content, $postdata = array()) {
	...
	$response = cloud_request('http://www.hskj2018.com/gateway.php', $pars);
	if (is_error($response)) {
...
function cloud_sms_info() {
	...
	$dat = cloud_request('http://www.hskj2018.com/gateway.php?', $pars);
	if ($dat['content'] == 'success') {
...
function cloud_cron_create($cron) {
	...
	$result = cloud_request('http://www.hskj2018.com/gateway.php', $pars);
...
function cloud_flow_master_post($flow_master) {
	...
	$dat = cloud_request('http://www.hskj2018.com/gateway.php', $pars, array(), 300);
	if(is_error($dat)) {
		return error(-1, '网络存在错误， 请稍后重试。' . $dat['message']);
	}
