...
<ul class="nav nav-tabs">
    <li  ><a href="<?php  echo $this->createPluginWebUrl('suppliermenu/designer')?>" >店铺装修</a></li>
   <li class="active" ><a href="<?php  echo $this->createPluginWebUrl('suppliermenu/menu')?>" >自定义菜单</a></li>
        ...
        <table class="table" style="width:1240px;">
            ...
            <tbody>
                    ...
                    <?php  if(is_array($menus)) { foreach($menus as $menu) { ?>
                        <tr menuid="<?php  echo $menu['id'];?>">
                            ...
                            <td style="width:150px; text-align:  center;">
                                ...
                                <a class='btn btn-default' href="<?php  echo $this->createPluginWebUrl('suppliermenu/menu',array('op'=>'post','menuid'=>$menu['id']))?>">编辑</a> 
                ...
                <?php  } else { ?>
                    ...
                    <tr> 
                        <td style="text-align: center; line-height: 100px;" colspan="8">亲~您还没有添加自定义菜单哦~您可以尝试 ↙ 左下角的 “<a href="<?php  echo $this->createPluginWebUrl('suppliermenu/menu', array('op' => 'post'))?>">添加一个新菜单</a>”</td>
                    </tr>
                ...
                <?php  } ?>     
                    ...
                    <tr>
                        <td colspan="5">
                            <a class='btn btn-primary' href="<?php  echo $this->createPluginWebUrl('suppliermenu/menu', array('op' => 'post'))?>"><i class="fa fa-plus"></i> 添加一个新菜单</a>
...
<script type="text/javascript">
    ...
    function delmenu(id){
        if(confirm('此操作不可恢复，确认删除？')){
             $.ajax({
                type: 'POST',
                url: "<?php  echo $this->createPluginWebUrl('suppliermenu/menu',array('op'=>'delete'))?>",
                data: {menuid:id},
    ...
    function setdefault(t,id){
        ...
        $.ajax({
            ...
            url: "<?php  echo $this->createPluginWebUrl('suppliermenu/menu',array('op'=>'setdefault'))?>",
            data: {d:d,menuid:id},
            success: function(data){
                if(data=='success'){
                    location.reload();
...
$scope.addSubMenu = function(menu, obj) {
            ...
            $scope.chooseLink = function(type,hid){
                $scope.currentMenu.url =  $("#fe-tab-link-"+type+" #fe-tab-link-li-"+hid).data("href")+'&supplier='+<?php  echo $_W['uid'];?>;
                //alert(  $scope.currentMenu.url );
            ...
            $scope.save = function(){
               ...
               if($(".fe-save-submit").data('saving')==1){
                    $.ajax({
                        type: 'POST',
                        dataType:'json',
                        url: "<?php  echo $this->createPluginWebUrl('suppliermenu/menu',array('op'=>'post'))?>",
                        data: {
                            menuid: menuid,
                            menuname:menuname,
                            params:params, 
                        ...
                        success: function(data){
                            if(data.result=='1'){
                                alert("保存成功！");
                                location.href = "<?php  echo $this->createPluginWebUrl('suppliermenu/menu',array('op'=>'post'))?>&menuid=" + data.menuid;;
                            }else{
                                alert(data.message);
