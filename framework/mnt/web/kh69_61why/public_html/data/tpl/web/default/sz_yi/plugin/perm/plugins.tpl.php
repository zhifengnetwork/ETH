        ...
        <div class="form-group form-plugins">
          ...
          <div class="col-sm-9 col-xs-12">
            ...
            <label class='checkbox-inline' style='margin:0;margin-right:5px;'>
              <input type='checkbox' name='plugins[]' class='plugin-item' value='<?php  echo $plugin['identity'];?>' <?php  if(in_array($plugin['identity'],$item_plugins)) { ?> checked<?php  } ?> />
...
<div class='panel panel-default'>
  ...
  <div class="">
    <table class="table">
      ...
      <tr>
        <td> <?php  if(!empty($row['username']) && empty($row['type']) ) { ?>用户: <?php  echo $row['username'];?><?php  } ?>
          <?php  if(!empty($row['name']) && $row['type']==1) { ?><?php  echo $row['name'];?><?php  } ?>
          <?php  if(empty($row['type'])) { ?>
          <label class='label label-default'>失效</label>
          <?php  } ?> </td>
        <td> <?php  if(is_array($row['plugins'])) { foreach($row['plugins'] as $p) { ?>
          <?php  echo $p['name'];?>;
          <?php  } } ?> </td>
