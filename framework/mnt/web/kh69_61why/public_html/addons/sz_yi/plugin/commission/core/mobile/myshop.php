...
if ($op == 'display') {
	if ($_W['isajax']) {
		if (empty($shop['selectgoods'])) {
			$arrlist1 = pdo_fetchall('select * from ' . tablename('sz_yi_goods') . ' where uniacid=:uniacid and isnew=1 and status=1 and deleted=0', array(':uniacid' => $_W['uniacid']));
			$arrlist2 = pdo_fetchall('select * from ' . tablename('sz_yi_goods') . ' where uniacid=:uniacid and ishot=1 and status=1 and deleted=0', array(':uniacid' => $_W['uniacid']));
			$arrlist3 = pdo_fetchall('select * from ' . tablename('sz_yi_goods') . ' where uniacid=:uniacid and isrecommand=1 and status=1 and deleted=0', array(':uniacid' => $_W['uniacid']));
			$arrlist4 = pdo_fetchall('select * from ' . tablename('sz_yi_goods') . ' where uniacid=:uniacid and isdiscount=1 and status=1 and deleted=0', array(':uniacid' => $_W['uniacid']));
			$arrlist5 = pdo_fetchall('select * from ' . tablename('sz_yi_goods') . ' where uniacid=:uniacid and issendfree=1 and status=1 and deleted=0', array(':uniacid' => $_W['uniacid']));
		} else {
		...
		$ret = array('shop' => $shop, 'arrlist1'=>$arrlist1, 'arrlist2'=>$arrlist2, 'arrlist3'=>$arrlist3, 'arrlist4'=>$arrlist4, 'arrlist5'=>$arrlist5, 'goodscount' => number_format($goodscount, 0), 'set' => m('common')->getSysset('shop'), 'advs' => $advs);
