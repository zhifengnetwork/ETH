...
if ($_W['isajax']) {
	...
	if ($_W['ispost']) {
		$orderids = array();
				$urlse=pdo_fetch('select * from ' .tablename('sz_yi_member'). ' where uniacid=:uniacid and openid=:openid',array(
		':uniacid'=>$_W['uniacid'],
		':openid'=>$openid
		));
		...
		$yuanguo=$urlse['yuanguo']+$apply['commission'];
		$yuanguo=array(
		'yuanguo'=>$yuanguo
		);
		//拿出自己的原果
		pdo_update('sz_yi_member',$yuanguo,array('id'=>$urlse['id']));
