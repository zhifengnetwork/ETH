    ...
    class YipinyimaModel extends PluginModel
    {
		...
		*/
		function tupian($path){
			...
			//生成随机zip
			$suiji=date('YmdHis') .".zip";
			
		
			$zip=new ZipArchive();

			if($zip->open($mkf ."/".$suiji, ZipArchive::OVERWRITE)=== TRUE){
		...
		*/
        ...
        /**
			...
			function glds(){
				...
				$erweibianhao=$this->get_sn();
				$path = IA_ROOT . "/addons/sz_yi/data/qrcodee/".$erweibianhao."/". $_W['uniacid']; 
				if (!is_dir($path)) {
                load()->func('file');
                mkdirs($path);
            }
            ...
            function yipinyuemmm($openid,$zjxz =0,$xjz =0,$msk =0,$name=0,$erwzt=0,$phost)
            {
                ...
                m('message')->sendCustomNotice($openid, $msg, $detailurl);
                    ...
                    $result = m('finance')->pay($openid, $zjxz, $jines);
                    if (is_error($result)) {
