...
$dis_level2 = pdo_fetchall("select dm.agentlevel,dm.nickname,l.thumb,l.commission_level from " . tablename('sz_yi_member') . " dm " . " left join " . tablename('sz_yi_dis_level') . " l on l.commission_level = dm.agentlevel" . " where dm.agentlevel= l.commission_level and dm.id = " . $id1 . "  and  dm.uniacid = " . $_W['uniacid'] . "  ORDER BY dm.agenttime desc");
/* print_r($dis_level2 ); */
...
if ($operation == 'display') {
    ...
    if (checksubmit('submit')) {
        ...
        plog('dis.agent.edit', "修改分销商 <br/>分销商信息:  ID: {$member['id']} /  {$member['openid']}/{$member['nickname']}/{$member['realname']}/{$member['mobile']}");
