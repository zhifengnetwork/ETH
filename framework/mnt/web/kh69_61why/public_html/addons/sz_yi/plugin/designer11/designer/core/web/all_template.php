...
if ($op == 'display') 
{
    ...
    $pages    = pdo_fetchall("SELECT * FROM " . tablename('sz_yi_designer') . " WHERE grade = 0 and (uniacid= :uniacid or uniacid = 0 or id=:id) and pagename LIKE :name " . "ORDER BY savetime DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(
        ':uniacid' => $_W['uniacid'],
        ':name'    => "%{$kw}%",
        ':id'      => $_W['pageid']
    ));
    $pagesnum = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('sz_yi_designer') . " WHERE grade = 0 and (uniacid= :uniacid or uniacid = 0) " . "ORDER BY savetime DESC ", array(
        ':uniacid' => $_W['uniacid']
    ));
    $total    = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('sz_yi_designer') . " WHERE grade = 0 and (uniacid= :uniacid or uniacid = 0) and pagename LIKE :name " . "ORDER BY savetime DESC ", array(
        ':uniacid' => $_W['uniacid'],
        ':name' => "%{$kw}%"
    ));
...
elseif ($op == 'post') 
{
    ...
    // var_dump($menus);die;
    $pages     = pdo_fetchall("SELECT id,pagename,pagetype,setdefault FROM " . tablename('sz_yi_designer') . " WHERE id= :id  ", array(
        ':id' => $pageid
    ));
    ...
    if (!empty($pageid)) {
        ...
        $datas = pdo_fetch("SELECT * FROM " . tablename('sz_yi_designer') . " WHERE (uniacid= :uniacid or uniacid= 0) and id=:id", array(
            ':uniacid' => $_W['uniacid'],
            ':id' => $pageid
        ));
...
elseif ($op == 'api') {
    ...
    if ($_W['ispost']) {
        ...
        if ($apido == 'savepage') {
            ...
            if (empty($pageid) or ($pageid < 25) && ($pageid == 25)) {
...
include $this->template('all_template');