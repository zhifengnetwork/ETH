...
$set = $this->getSet();
//报单等级
	//$bd_level22 = pdo_fetchall("select id,levelname from  " . tablename('bd_level') . " where uniacid=:uniacid  order by id asc " , array('uniacid' => $_W['uniacid']));
/* if($operation == 'display'){

	if (!empty($_GPC['agentlevel'])) {
        $condition .= ' and bd_level=' . intval($_GPC['agentlevel']);
    ...
    $psize2     = 20;
    $params2    = array();
	$sql2 = "select dm.id,dm.realname,dm.bd_level,bm.levelname from " . tablename('sz_yi_dis_level') . " dm " . " left join " . tablename('bd_level') . " bm on bm.id = dm.bd_level"  . " where   dm.uniacid = " . $_W['uniacid'] . "  and bm.uniacid = " . $_W['uniacid'] . "  {$condition} ORDER BY dm.id desc";
	...
	if($_POST){
			...
			if(!empty($c_leveluid)){
					...
					//获取等级的id
					foreach($cm_level as $key=>$va){
						$_var_1[$key] = $va['commission_level'];
					
					}
					foreach(array_unique($_var_1) as $ke=>$v){
						$clevel = pdo_fetch("SELECT bg,data,commission_level FROM " . tablename('sz_yi_dis_clevel') . " WHERE  uniacid=:uniacid and  commission_level=:commission_level order by id asc ", array( ':uniacid' => $_W['uniacid'],':commission_level' =>$v));
						$_var_2[$ke] = $clevel;
					}
					...
					if(!empty($result)){
						...
						//循环等级的ID
						 foreach($_var_2 as $k=>$val1){

						foreach($result as $result_id){
						$member = pdo_fetchall("SELECT id,realname,mobile,weixin,avatar,agentlevel FROM " . tablename('sz_yi_member') . " WHERE  uniacid=:uniacid   and id=:id  and agentlevel=:agentlevel", array( ':uniacid' => $_W['uniacid'],':id' => $result_id ,':agentlevel' =>$val1['commission_level']));
						...
						if(!empty($member)){
								...
								foreach($member as $row){
										 $data = array(
											...
											$data11 = json_decode(str_replace('&quot;', "'", $val1['data']), true);
									 	...
									 	if(!empty($val1['bg'])){
												...
												$dst_path = ATTACHMENT_ROOT.$val1['bg'];//背景图
			...
			if(!empty($b_leveluid)){
					...
					//获取等级的id
					foreach($bo_level as $key=>$va_1){
						$_var_2[$key] = $va_1['bonus_level'];
					
					}
					
					foreach(array_unique($_var_2) as $ke=>$v_1){
						
						$blevel = pdo_fetch("SELECT id,bg,data,bonus_level FROM " . tablename('sz_yi_dis_blevel') . " WHERE  uniacid=:uniacid and  bonus_level=:bonus_level order by id asc ", array( ':uniacid' => $_W['uniacid'],':bonus_level' =>$v_1));
						$_var_2b[$ke] = $blevel;
					}
					...
					if(!empty($result2)){
						...
						//循环等级的ID
						 foreach($_var_2b as $k=>$val1_2){
						 
						foreach($result2 as $result_id){
						
						$member = pdo_fetchall("SELECT id,realname,mobile,weixin,avatar,bonuslevel FROM " . tablename('sz_yi_member') . " WHERE  uniacid=:uniacid   and id=:id  and bonuslevel=:bonuslevel", array( ':uniacid' => $_W['uniacid'],':id' => $result_id ,':bonuslevel' =>$val1_2['bonus_level']));
						...
						if(!empty($member)){
								...
								foreach($member as $row){
										 $data = array(
											...
											$data11 = json_decode(str_replace('&quot;', "'", $val1_2['data']), true);
									 	...
									 	if(!empty($val1_2['bg'])){
												...
												$dst_path = ATTACHMENT_ROOT.$val1_2['bg'];//背景图
												 $dst = imagecreatefromstring(file_get_contents($dst_path));//创建图片的实例
												 $font = dirname(__FILE__).'/simsun.ttc';//字体
												 list($dst_w, $dst_h) = getimagesize($dst_path);
												//判断是不是添加图片水印
			...
			if(!empty($bd_leveluid)){
					...
					//获取等级的id
					foreach($bd_level as $key=>$va_bd){
						$_var_bd[$key] = $va_bd['bd_level'];
					
					}
					foreach(array_unique($_var_bd) as $ke=>$v_bd){
						
						$bdlevel = pdo_fetch("SELECT id,bg,data,bd_level FROM " . tablename('sz_yi_dis_level') . " WHERE  uniacid=:uniacid and  bd_level=:bd_level order by id asc ", array( ':uniacid' => $_W['uniacid'],':bd_level' =>$v_bd));
						$_var_2bd[$ke] = $bdlevel;
					}
					...
					if(!empty($result2)){
						//取原本data字段的值 
						//循环等级的ID
					  foreach($_var_2bd as $k=>$val1_bd){
						 
						foreach($result2 as $result_id){
						$member = pdo_fetchall("select m.id,m.realname,m.mobile,m.weixin,m.avatar,bm.level as level1 from " . tablename('sz_yi_member') . " m" . " left join " . tablename('bd_member') . " bm on bm.openid = m.openid"  . " where  m.uniacid=:uniacid and bm.uniacid=:uniacid  and m.id=:id    and bm.level=:level", array( ':uniacid' => $_W['uniacid'],':id' => $result_id ,':level' =>$val1_bd['bd_level']));
						...
						if(!empty($member)){
								...
								foreach($member as $row){
										 $data = array(
											...
											$data11 = json_decode(str_replace('&quot;', "'", $val1_bd['data']), true);
									 	...
									 	if(!empty($val1_bd['bg'])){
												...
												$dst_path = ATTACHMENT_ROOT.$val1_bd['bg'];//背景图
												 $dst = imagecreatefromstring(file_get_contents($dst_path));//创建图片的实例
												 $font = dirname(__FILE__).'/simsun.ttc';//字体
												 list($dst_w, $dst_h) = getimagesize($dst_path);
												//判断是不是添加图片水印
