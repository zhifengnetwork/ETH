...
if (!class_exists('ReturnModel')) {
	...
	class ReturnModel extends PluginModel
	{
		...
		public function selectreturn($orderid){
		...
		public function setGoodsQueue($orderid) {
			...
			$good = pdo_fetch("SELECT og.orderid,og.goodsid,og.total,og.price,g.isreturnqueue,o.openid,m.id as mid,g.isreturn FROM " . tablename('sz_yi_order') . " o left join " . tablename('sz_yi_member') . " m  on o.openid = m.openid left join " . tablename("sz_yi_order_goods") . " og on og.orderid = o.id  left join " . tablename("sz_yi_goods") . " g on g.id = og.goodsid WHERE o.id = :orderid and o.uniacid = :uniacid and m.uniacid = :uniacid",
				array(':orderid' => $orderid,':uniacid' => $_W['uniacid']
			));
		...
		public function returnlist($queueid){
			...
			$status1 = pdo_fetchcolumn("select count(q.queue) from ".tablename('sz_yi_order_goods_queue') . " q left join " .  tablename('sz_yi_order') ." o on o.id = q.orderid " . " where o.uniacid='".$_W['uniacid']."'     and q.goodsid='". $goods_queue['goodsid'] . "' and o.status =3  and q.type=0 order by q.id asc ");
					...
					for($a ; $a<=$num; ){
							...
							if($a >= $status['queue'] * $_var_1){
								//$list = pdo_fetch("select * from ".tablename('sz_yi_order_goods_queue')." where uniacid='".$_W['uniacid']."'   and goodsid='".$goods_queue['goodsid']."'    and type='0' order by id asc limit 1");	
									...
									$list = pdo_fetch("select q.openid as openid1,q.id as id1,q.price as price1 from ".tablename('sz_yi_order_goods_queue')."q left join" . tablename('sz_yi_order'). " o on o.id = q.orderid " . " where o.uniacid='".$_W['uniacid']."' and o.status =3 and q.type='0' order by q.id asc limit 1");	
						...
						$return_day = date('Ymd',$return_money['createtime']);
						 ...
						 if($day != $return_day){
								foreach($return as $val){
										$return_money = $val['return_money'] * $set['percentage']/100;//计算返金额
										   ...
										   if(empty($member['uid'])){
													//member表
														if($spare !=0){
															...
															pdo_insert('mc_credits_record',$mc_credits_record);
