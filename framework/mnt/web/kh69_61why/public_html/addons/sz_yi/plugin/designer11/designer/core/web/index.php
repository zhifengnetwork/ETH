...
if ($op == 'display') 
{
    ...
    $pages    = pdo_fetchall("SELECT * FROM " . tablename('sz_yi_designer') . " WHERE grade = 1 and (uniacid= :uniacid or uniacid = 0) and pagename LIKE :name " . "ORDER BY savetime DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(
        ':uniacid' => $_W['uniacid'],
        ':name' => "%{$kw}%"
    ));
    $pagesnum = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('sz_yi_designer') . " WHERE grade = 1 and (uniacid= :uniacid or uniacid = 0) " . "ORDER BY savetime DESC ", array(
        ':uniacid' => $_W['uniacid']
    ));
    $total    = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('sz_yi_designer') . " WHERE grade = 1 and (uniacid= :uniacid or uniacid = 0) and pagename LIKE :name " . "ORDER BY savetime DESC ", array(
        ':uniacid' => $_W['uniacid'],
        ':name' => "%{$kw}%"
    ));
