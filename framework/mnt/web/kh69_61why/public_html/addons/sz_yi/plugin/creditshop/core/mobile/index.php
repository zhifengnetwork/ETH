...
if ($_W['isajax']) {
	...
	$times = pdo_fetchall("select id,title,subtitle,credit,thumb,type from " . tablename('sz_yi_creditshop_goods') . ' where uniacid=:uniacid and istime=1 and status=1 and deleted=0  order by displayorder desc ', array(':uniacid' => $uniacid));
	$times = set_medias($times, 'thumb');
	$recommands = pdo_fetchall("select id,title,credit,subtitle,thumb,type from " . tablename('sz_yi_creditshop_goods') . ' where uniacid=:uniacid and isrecommand=1 and status=1 and deleted=0  order by displayorder desc ', array(':uniacid' => $uniacid));
	$recommands = set_medias($recommands, 'thumb');
	$member = m('member')->getMember($openid);
	$groupid = intval($member['groupid']);
	$levelid = intval($member['level']);
	$vips = pdo_fetchall("select id,title,credit,subtitle,thumb,type from " . tablename('sz_yi_creditshop_goods') . " " . " where uniacid=:uniacid " . " and ( FIND_IN_SET( {$levelid},showlevels)<>0 or FIND_IN_SET( {$groupid},showgroups)<>0 ) " . " and status=1 and deleted=0 order by displayorder desc ", array(':uniacid' => $uniacid));
	$vips = set_medias($vips, 'thumb');
	$reccategory = array();
	foreach ($category as $c) {
		if (!empty($c['isrecommand'])) {
			$goods = pdo_fetchall("select id,title,subtitle,credit,thumb,type from " . tablename('sz_yi_creditshop_goods') . ' where uniacid=:uniacid and cate=:cate and isrecommand=1 and status=1 and deleted=0 order by displayorder desc ', array(':uniacid' => $uniacid, ":cate" => $c['id']));
