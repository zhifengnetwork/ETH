...
if ($operation == 'display')
	...
	$id=intval($_GPC['id']);//接受id值
	//查询两个表
	  $list = pdo_fetchall("SELECT * FROM " . tablename('yipinyima') . " d " .
        " left join " . tablename('yipinyimaerwei') . ' m on m.yipin_id = d.id ' .
        " where d.id=" . $id . "  and d.uniacid=" . $_W['uniacid'] . "");
	//得到所有的数据
	
	if(!empty($list)){
	
		$path = IA_ROOT .  "/ewm/".$_W['uniacid']."/" . date('Ymd');
		if(!is_dir($path)){
			@mkdir($path,0777,true);
		}
		foreach($list as $key=>$var){
			$imgsf=$var['erweiurl'];
			$test=$var['erweibianhao'];
			$file=IA_ROOT .$imgsf;
			$newfile=$path ."/" .$test .".png";
			$return=copy($file,$newfile);
		}
		$url=$this->model->tupian($path);
		$downurl = str_replace(IA_ROOT,'',$url);
