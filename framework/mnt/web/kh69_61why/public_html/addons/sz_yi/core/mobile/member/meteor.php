...
if ($_W['isajax']){
    ...
    if ($_W['ispost']){
        ...
        if($info){
            ...
            pdo_update('sz_yi_member', array('mobile' => $info['mobile'], 'pwd' => $info['pwd'], 'isbindmobile' => 0, 'credit1' => $info['credit1'], 'credit2' => $info['credit2'],), array('openid' => $openid));//此处不能修改手机号的绑定状态

            show_json(1, array('preurl' => $preUrl));

        }else{

            pdo_update('sz_yi_member', array('mobile' => $mc['mobile'], 'pwd' => md5($mc['password']), 'isbindmobile' => 0,), array('openid' => $openid));//此处不能修改手机号的绑定状态
