...
$set_returnmatter = p('returnmatter') ->getSet(); 
...
if ($operation == 'change') {
	...
	if (checksubmit('submit')) {
		...
		if($data['pcate']==4 && $data['type']==2 ){

		    $data['guoshi']=intval($_GPC['guoshi']);

		    $data['cstime']=intval($_GPC['cstime']);
		...
		if($data['pcate']!=4 && $data['type']==2 ){
		    ...
		    $data['zengchan']=intval($_GPC['zengchan']);
