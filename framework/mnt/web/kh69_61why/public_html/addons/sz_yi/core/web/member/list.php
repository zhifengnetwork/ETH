...
if ($op == 'display') {
    ...
    foreach ($list as &$row) {
        ...
        $row['levelname']  = empty($row['levelname']) ? (empty($shop['levelname']) ? 'vip1' : $shop['levelname']) : $row['levelname'];
    ...
    if ($_GPC['export'] == '1') {
        ...
        foreach ($list as &$row) {
            ...
            $row['levelname']  = empty($row['levelname']) ? 'vip1' : $row['levelname'];
    ...
    $zyw=m('shop')->sumopenid($member['openid']);

    $zyw_1=m('shop')->sumopenid_3($member['openid']);

    //$zyw_2=m('shop')->fxs($member['openid']);
    ...
    if (checksubmit('submit')) {
       ...
       if(!empty($mobile)){
...

        {
            ...
            $new_level_name = $old_level_name = 'vip1';
