...
if ($_W['isajax']) {
    ...
    if ($operation == 'display') {
        ...
        if ($sale_plugin) {
        	...
        	if($goods[0]['type']== 1){ 
        		//$credit = m('member')->getCredit($openid, 'xianguo');
        		$credit = pdo_fetchcolumn("SELECT xianguo FROM ".tablename("sz_yi_member")." WHERE uniacid=:uniacid AND openid=:openid",array(":uniacid"=>$_W['uniacid'],":openid"=>$openid));
        	}else if($goods[0]['type']== 2){ 
				$credit = pdo_fetchcolumn("SELECT gengzhongguo FROM ".tablename("sz_yi_member")." WHERE uniacid=:uniacid AND openid=:openid",array(":uniacid"=>$_W['uniacid'],":openid"=>$openid));
        		//$credit = m('member')->getCredit($openid, 'gengzhongguo');
        	}
            ...
            if (!empty($saleset['creditdeduct'])) {
                ...
                if ($deductmoney > $deductprice*$total) {



                    $deductmoney = $deductprice*$total;
        ...
        $zyw_dikou=0;
        foreach ($goods as $v){
            $zyw_dikou+=$v['deduct']*$v['total'];
        }

        if($money1>$zyw_dikou){
            $money1=$zyw_dikou;
            $deductcredit=$zyw_dikou;
            $deductmoney=$zyw_dikou;
        }
        show_json(1, array(
...
$prc = $_GPC['prc'];
        ...
        foreach ($goodsarr as $g) {
            ...
            if(p('supplier')){
                ...
                $sql  = 'SELECT  id as goodsid,pcate,ccate,title,type, weight,total,issendfree,isnodiscount, isreturn,returnmatter,thumb,marketprice,cash,isverify,goodssn,productsn,sales,istime,timestart,timeend,usermaxbuy,maxbuy,unit,buylevels,buygroups,deleted,status,deduct,manydeduct,virtual,discounts,deduct2,ednum,edmoney,edareas,diyformtype,diyformid,diymode,dispatchtype,dispatchid,dispatchprice FROM ' . tablename('sz_yi_goods') . ' where id=:id and uniacid=:uniacid  limit 1';
    ...
    $sql="select agentlevel from ".tablename('sz_yi_member')." where openid='".$openid."'";
    $agent_level=pdo_fetchcolumn($sql);
       $zyw = 0 ;
     
        if($data['type']==2  && $data['pcate']==4 ){

           $zyw_20= m('order')->getTudi($openid);

           if($zyw_20==0){

               show_json(5,"您没有种植的土地了,快分享商城赚取土地吧!");

           }

            $zyw=1;//种子

        }else if($data['type']==2  && $data['pcate']==1){
            if($agent_level<1 && $data['goodsid']==9){
                //黄土地
                show_json(-1,"您的等级不够！");
            }
            if($agent_level<2 && $data['goodsid']==17){
                //红土地
                show_json(-1,"您的等级不够！");
            }
            if($agent_level<3 && $data['goodsid']==10){
                //蓝土地
                show_json(-1,"您的等级不够！");
            }
            if($agent_level<4 && $data['goodsid']==11){
                //紫土地
                show_json(-1,"您的等级不够！");
            }
            if($agent_level<5 && $data['goodsid']==12){
                //金土地
                show_json(-1,"您的等级不够！");
            }
        ...
        }else if($data['goodsid']==8){
            $sql="select * from ".tablename('sz_yi_tudi_1')." where openid='".$openid."' and status=1 and shifei=0";
              $zyw=pdo_fetchall($sql);
            if(empty($zyw)){
                show_json(-1,"您的土地不需要施肥");
            }

            $zyw=3;//施肥

        }else if($data['type']==2  && $data['pcate']==6 && $data['goodsid']==5){

            $zyw=4;//狗
            $sql="select * from ".tablename('sz_dog')." where openid='".$openid."'";
            $a=pdo_fetch($sql);
            if(!empty($a)){
                show_json(-1,"您已经有狗了，不能重新购买");
            }

        } else if($data['goodsid']==6){
            //狗粮
           $sql="select * from ".tablename('sz_dog')." where openid='".$openid."'";
           $a=pdo_fetch($sql);
           if(!empty($a)){
               if($a['level']==1){
                   ...
                   show_json(-1,"您的狗狗吃不了这个狗粮");
               }
           }else{
               show_json(-1,"您还没有狗狗，不能购买");
           }
        }else if($data['goodsid']==7){
            //狗王粮
            $sql="select * from ".tablename('sz_dog')." where openid='".$openid."'";
            $a=pdo_fetch($sql);
            if(!empty($a)){
        ...
        }else if($data['goodsid']==19){
            //管家
            $sql="select status from ".tablename('sz_yi_guanjia')." where openid='".$openid."'";
            $a=pdo_fetchcolumn($sql);
            if(!empty($a)){
                    show_json(-1,"您已经有管家了，不需要重新购买");
        ...
        $order    = array(
            ...
            'zyw'=>$zyw,      "couponprice" => $couponprice,

			'returnmatter' =>$data['returnmatter']
