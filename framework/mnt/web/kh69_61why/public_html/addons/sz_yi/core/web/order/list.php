...
if ($operation == 'display') {
	...
	foreach($list as $key=>$value){
		if($value['status']==2){
			...
			$day = $coum/86400;
			if($day>=7){
				print_r($list[$key]['status']);
				pdo_update('sz_yi_order', array('status' => 3),array('id' => $list[$key]['id'], 'uniacid' => $_W['uniacid']));
				
					$_var_0 = p('return')->getSet();
				if(!empty($_var_0['out'])){
					 //全返
					 p('return') -> setGoodsQueue($_var_21['id']);
				}
				if($_var_0['out']==0){
	...
	$paytype = array('0' => array('css' => 'default', 'name' => '未支付'),'100' => array('css' => 'success', 'name' => '微信支付'), '1' => array('css' => 'danger', 'name' => '余额支付'), '11' => array('css' => 'default', 'name' => '后台付款'), '2' => array('css' => 'danger', 'name' => '在线支付'), '21' => array('css' => 'success', 'name' => '微信支付'), '22' => array('css' => 'warning', 'name' => '支付宝支付'), '23' => array('css' => 'warning', 'name' => '银联支付'), '3' => array('css' => 'primary', 'name' => '货到付款'),);
	...
	if ($to == 'confirmpay') {
		...
		//购物返
		p('returnmatter') -> addreutrn($_var_21['id']);
		...
		$len = count($arr);
		foreach($arr as $k=>$v){
			$list[] = array('time'=>$v['time'],'step'=>$v['context'],'ts'=> strtotime(trim($v['time'])));
		}
		load()->func('tpl');
...
{
	$_var_4="https://m.kuaidi100.com/query?postid=$_var_3&type=$_var_2";
	...
	$_var_8=json_decode($_var_6);
	
	$var8 =  (array)$_var_8;
	foreach($var8['data'] as &$v){
		$v = (array)$v;
	}
	unset($v);
