...
$args = array('pagesize' => 20, 'page' => $_GPC['page'], 'isnew' => $_GPC['isnew'], 'ishot' => $_GPC['ishot'], 'isrecommand' => $_GPC['isrecommand'], 'isdiscount' => $_GPC['isdiscount'], 'istime' => $_GPC['istime'], 'keywords' => $_GPC['keywords'], 'pcate' => $_GPC['pcate'], 'ccate' => $_GPC['ccate'], 'tcate' => $_GPC['tcate'], 'order' => $_GPC['order'], 'by' => $_GPC['by'],'supplier_uid'=>$supplier);
...
$total = m('goods') -> tatol($args);
//$total = pdo_fetchcolumn('SELECT count(*) FROM ' . tablename('sz_yi_goods') . " where 1 {$condition}", $params);
//echo 'SELECT count(*) FROM ' . tablename('sz_yi_goods') . " where 1 {$condition}";
