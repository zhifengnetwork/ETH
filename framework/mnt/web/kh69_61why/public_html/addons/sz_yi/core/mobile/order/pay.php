...
if ($operation == 'display' && $_W['isajax']){
    ...
    $pay = array('success'=>false);

    $pay['success'] = $setting['payment']['wtpay']['switch'];
    ...
    show_json(1, array('order' => $order, 'set' => $set, 'credit' => $credit, 'wechat' => $wechat,'pay'=>$pay, 'alipay' => $alipay, 'unionpay' => $unionpay, 'yunpay' => $yunpay, 'cash' => $cash, 'isweixin' => is_weixin(), 'currentcredit' => $currentcredit, 'returnurl' => $returnurl, 'goods' => $order_goods));
    ...
    $type = $_GPC['type'];
    if (!in_array($type, array('weixin', 'alipay', 'unionpay', 'yunpay','pay'))){
        show_json(0, '未找到支付方式');
    }
    ...
    if ($type == 'weixin'){
     ...
     //       'total_fee'=>100,
            ...
            'notify_url' => $_W['siteroot'].'addons/sz_yi/payment/pay/notify.php',
            'callback_url' => $_W['siteroot'].'app/index.php?i='.$_W['uniacid']."&c=entry&do=shop&m=sz_yi",
        );
               
        $result = m('request')->submitOrderInfo( $payData );
        
        if(!empty($result['token_id'])){
            $payUrl .="?token_id={$result['token_id']}&showwxpaytitle=1";
        }

        pdo_update('sz_yi_order', array('paytype' => 100), array('id' => $order['id']));
