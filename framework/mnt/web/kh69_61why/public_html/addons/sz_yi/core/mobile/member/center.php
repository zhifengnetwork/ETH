...
$level = array('levelname' => empty($this->yzShopSet['levelname']) ? 'vip1' : $this->yzShopSet['levelname']);
...
if ($_W['isajax']) {
    ...
    $sql="select diymemberdata from ".tablename('sz_yi_member')." where openid='".$openid."'";

    $s_zyw= pdo_fetchcolumn($sql);
	...
	show_json(1, array('member' => $member, 'referrer' => $referrer,'zyw'=>$zyw,'shop_set' => $shop_set, 'order' => $order, 'level' => $level, 'open_creditshop' => $open_creditshop, 'counts' => $counts, 'shopset' => $shopset, 'trade' => $trade));
