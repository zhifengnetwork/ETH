...
$sql="select sum(price) from".tablename('sz_yi_order')." where status=3 and paytype=0";
$zyw_1=pdo_fetchcolumn($sql);
$sql="select sum(price) from".tablename('sz_yi_order')." where status>0 and paytype=100";
$zyw_11=pdo_fetchcolumn($sql);
$zyw_111=$zyw_1+$zyw_11;
$zyw=time();

$ezyw=strtotime(date("Y-m-d 0:0:0",time()));
/* print_r(date("Y-m-d H:i:s",$ezyw));die; */
 $sql="select sum(price) from ".tablename('sz_yi_order')." where paytype=100   and status>0 and   paytime >= ".$ezyw." AND paytime <= ".$zyw;
 $zyw_222=pdo_fetchcolumn($sql); 
if($do == 'chart') {
